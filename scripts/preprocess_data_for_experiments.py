...
def exp_ordinal_encoder_categoricas(
    ...
    transformer = make_column_transformer(
        (encoder, category_columns), remainder="passthrough"
    )
...
def exp_onehotencoder_categoricas(
    ...
    return (
        pd.DataFrame(
            X_train, columns=pipeline.get_feature_names_out(), index=train_idx
        ),
        pd.DataFrame(
            X_test, columns=pipeline.get_feature_names_out(), index=test_index
        ),
    )
